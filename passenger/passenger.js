...
/* =========================================
   BUS LINES DATABASE (Meknès Example)
========================================= */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  populateStops();
  getPassengerLocation();   // 🔥 THIS WAS MISSING
...
function populateStops() {
  ...
  BUS_LINES.forEach(line => {
    line.stops.forEach(stop => allStops.add(stop));
  });

  const stopsArray = Array.from(allStops).sort();
...
function findRoute() {
  ...
  if (from === to) {
    resultDiv.innerHTML =
      "<p style='color:red'>Departure and destination cannot be the same.</p>";
    return;
  }

  // Find direct lines
  const directLines = BUS_LINES.filter(line =>
    line.stops.includes(from) && line.stops.includes(to)
  );

  if (directLines.length > 0) {

    resultDiv.innerHTML = "<h4>Direct Lines Available:</h4>";
    ...
    directLines.forEach(line => {
      ...
      if (fromIndex < toIndex) {
        resultDiv.innerHTML += `
          <div style="padding:8px; background:#e8f5e9; margin-bottom:6px; border-radius:6px;">
            🚌 ${line.name} – Direct route
          </div>
        `;
      }
    });
    ...
    BUS_LINES.forEach(line1 => {
      if (line1.stops.includes(from)) {

        BUS_LINES.forEach(line2 => {

          if (line2.stops.includes(to) && line1 !== line2) {

            const commonStops = line1.stops.filter(stop =>
              line2.stops.includes(stop)
            );
            ...
            if (commonStops.length > 0) {
              ...
              resultDiv.innerHTML = `
                <h4>Transfer Required:</h4>
                🚌 ${line1.name} → Change at ${commonStops[0]} → 🚌 ${line2.name}
              `;
            }
          }
        });
    ...
    if (!transferFound) {
      resultDiv.innerHTML =
        "<p style='color:red'>No available route found.</p>";
    }
  }
}
...
function placeBus() {
  ...
  const busLineNumber =
  localStorage.getItem("selectedLine") || "Line 5";
...
if (remainingDistanceKm < userAlertDistance) {
